...
# Output two programs: a sed script to perform tokenization, and an external
# mode for John the Ripper to perform the reverse translation.
...
for (my $len = $maxlen; $len >= $minlen; $len--) {
	for (my $i = 0; $i <= $#tokens; $i++) {
		...
		$sed .= '; ' if ($sed);
		$sed .= sprintf("s/$tokens[$i]/\\x%x/", $subst[$i]);
	}
}
